...
struct AppGate: View {
    ...
    var body: some View {
        Group {
            ...
            case .launching:
                ProgressView("Biztonságos munkamenet ellenőrzése…")
            ...
            case .configurationError(let detail):
                VStack(spacing: 18) {
                    Image(systemName: "exclamationmark.shield").font(.system(size: 52)).foregroundStyle(.red)
...
struct PreviewNotice: View {
    ...
    var body: some View {
        HStack(alignment: .firstTextBaseline) {
            Label(store.syncStatus.label,
                  systemImage: store.authStatus == .offline ? "wifi.slash" : "checkmark.shield")
            Spacer()
            if [.pending, .conflict, .failed].contains(store.syncStatus), store.isOnline {
                Button("Újra") { store.retrySync() }.buttonStyle(.borderless)
            }
        }
        .font(.footnote).foregroundStyle(store.syncStatus == .conflict ? .orange : .secondary)
